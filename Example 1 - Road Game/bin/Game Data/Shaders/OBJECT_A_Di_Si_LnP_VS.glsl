...
void main(void)
{
	...
	float distance = length(l_direction_C);
	ref_color = MaterialProperty.x*LightColor[0] + MaterialProperty.y*LightPower[0]*cosTheta*LightColor[0]/(distance * distance) + MaterialProperty.z*LightPower[0]*LightColor[0]*pow(cosAlpha, 10)/(distance*distance);
	UV = in_UV;
}
