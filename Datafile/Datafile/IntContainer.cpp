...
IntContainer::IntContainer(DatabaseTable::DatabaseTableEnum type) : DatabaseContainer<int>(type)
{
...
void IntContainer::Write(std::vector<char>* vec)
{
	unsigned long bytes=0;
	DataContainer<int>* val = 0;
	//Writes Header and writes all values to disk -- WRITES BACKWARDS
	if(_table.size() > 0)
	{
		std::deque<DataContainer<int>*>::reverse_iterator i = _table.rbegin();
		for(;i!= _table.rend(); i++)
		{
			val = *i;
			...
			int data = *val->Item();
			data = htonl(data);
			vec->insert(vec->end(), (char*)&data, (char*)&data + sizeof(int));
			bytes += sizeof(int);
		}
	}
	bytes = htonl(bytes);
...
void IntContainer::Read(std::vector<char>* vec, size_t* pos)
{
	...
	*pos = *pos - sizeof(unsigned long);
	...
	while(bytes > count)
	{
		*pos = *pos - sizeof(unsigned long);
		assert(*pos > 0);
		memcpy(&val, &vec->operator[](*pos), sizeof(int));
		val = ntohl(val);
