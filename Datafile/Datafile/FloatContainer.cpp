...
FloatContainer::FloatContainer(DatabaseTable::DatabaseTableEnum type) : DatabaseContainer<float>(type)
{
}
...
void FloatContainer::Write(std::vector<char>* vec)
{
	unsigned long bytes=0;
	DataContainer<float>* val = 0;
	//Writes Header and writes all values to disk -- WRITES BACKWARDS
	if(_table.size() > 0)
	{
		std::deque<DataContainer<float>*>::reverse_iterator i = _table.rbegin();
		for(;i!= _table.rend(); i++)
		{
			val = *i;
...
void FloatContainer::Read(std::vector<char>* vec, size_t* pos)
{
	...
	*pos = *pos - sizeof(unsigned long);
	...
	while(bytes > count)
	{
		*pos = *pos - sizeof(float);
		assert(*pos > 0);
		memcpy(&val, &vec->operator[](*pos), sizeof(float));
		d = ntohf(val);
		count += sizeof(float);
